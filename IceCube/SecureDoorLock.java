...
public class SecureDoorLock {
    ...
    private String getFullName(String firstName, String lastName) {
        ...
        for (int i = 0; i < firstName.length(); i++) {
        	char fName = firstNameLow.charAt(i);
        	code += alphabet.indexOf(fName);
        }
        for (int j = 0; j < lastName.length(); j++) {
        	char lName = lastNameLow.charAt(j);
        	code += alphabet.indexOf(lName);
        }
        if (code.equals("942464138221941715010")) {
        	return "Ice Cube";
        }
